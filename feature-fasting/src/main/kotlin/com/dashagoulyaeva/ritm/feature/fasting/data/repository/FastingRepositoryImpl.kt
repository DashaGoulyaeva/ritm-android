...
@Singleton
class FastingRepositoryImpl
    @Inject
    constructor(
        private val fastingDao: FastingDao,
    ) : FastingRepository {
        override fun getActiveSession(): Flow<FastingSession?> = fastingDao.getActiveSession().map { it?.toDomain() }

        override suspend fun getActiveSessionOnce(): FastingSession? = fastingDao.getActiveSessionOnce()?.toDomain()
        ...
        private fun FastingSessionEntity.toDomain() =
            FastingSession(
                id = id,
