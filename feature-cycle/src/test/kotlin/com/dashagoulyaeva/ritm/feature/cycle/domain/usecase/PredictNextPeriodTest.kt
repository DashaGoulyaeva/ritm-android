...
class PredictNextPeriodTest {
    private fun fakeRepo(periods: List<CyclePeriod>): CycleRepository =
    ...
    @Test
    fun `returns null when fewer than 2 completed periods`() =
        runTest {
            val repo =
                fakeRepo(
                    listOf(
                        CyclePeriod(1, "2026-01-01", null),
                    ),
                )
    ...
    @Test
    fun `returns null when only 1 completed period`() =
        runTest {
            val repo =
                fakeRepo(
                    listOf(
    ...
    @Test
    fun `predicts next period with 2 completed cycles of 28 days`() =
        runTest {
            val repo =
                fakeRepo(
                    listOf(
                        CyclePeriod(1, "2026-01-01", "2026-01-05"),
                        CyclePeriod(2, "2026-01-29", "2026-02-02"),
                    ),
                )
    ...
    @Test
    fun `predicts with average of multiple cycle lengths`() =
        runTest {
            val repo =
                fakeRepo(
                    listOf(
                        CyclePeriod(1, "2026-01-01", "2026-01-05"),
                        CyclePeriod(2, "2026-01-29", "2026-02-02"),
                        CyclePeriod(3, "2026-02-28", "2026-03-03"),
                    ),
                )
            val result = PredictNextPeriod(repo)()
