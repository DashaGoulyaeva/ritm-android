...
class GetHabitStreakTest {
    ...
    private fun fakeRepo(checks: List<HabitCheck>): HabitRepository = object : HabitRepository {
        ...
        override suspend fun checkHabit(habitId: Long, date: String) = Unit
        override suspend fun uncheckHabit(habitId: Long, date: String) = Unit
        override suspend fun isCheckedToday(habitId: Long, date: String): Boolean = false
    }

    @Test
    fun `streak is zero when no checks`() = runTest {
    ...
    @Test
    fun `streak counts consecutive days including today`() = runTest {
    ...
    @Test
    fun `streak is zero if today not checked`() = runTest {
        val today = LocalDate.now()
        val checks = listOf(
