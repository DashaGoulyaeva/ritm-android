...
class CheckHabitTodayTest {
    ...
    // Записывает вызовы checkHabit / uncheckHabit для проверки
    ...
    @Test
    fun `checked true calls checkHabit`() = runTest {
    ...
    @Test
    fun `checked false calls uncheckHabit`() = runTest {
        val spy = SpyRepository()
        val useCase = CheckHabitToday(spy)
        val today = LocalDate.now().toString()
    ...
    @Test
    fun `multiple calls accumulate correctly`() = runTest {
        val spy = SpyRepository()
        val useCase = CheckHabitToday(spy)
        val today = LocalDate.now().toString()
