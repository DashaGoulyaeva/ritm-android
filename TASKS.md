...
| # | ?????? | Owner | ?????? | ??????? ?? |
|---|---|---|---|---|
| D4-01 | ????????? ????? ??????? v4 ? ???????????? v4.1 | Docs | DONE | ????? 0-6 |
| D4-02 | Mini-spec ?????????? v4: Today, Steps, Fasting, ???????? ? ??????? MVP | Product | DONE | D4-01 |
| D4-03 | ADR/technical note: mapping v4 tokens ? Compose ? ????????? UI primitives | Architect | DONE | D4-02 |
| D4-04 | Core-ui primitives: rhythm colors, soft surfaces, RitmBanner, RhythmOrb | Builder | DONE | D4-03 |
| D4-05 | Today/Steps/Fasting v4: ????? ?????? ???, fallback ?????, ????? fasting-???? | Builder | DONE | D4-04 |
| D4-06 | Quality gate cleanup: ????????? ? ?????? core-database, ??????? ?????????? detekt-???? | Lead | DONE | D4-05 |
